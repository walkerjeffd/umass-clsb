...
# 20 Nov 2018: I was setting graph.kern[i] to zero when kernel runs out, which was breaking trim.along.graph
...

{
    ...
    for (i in e) {                                  # for each adjacent edge,
        v <- max(a - edgecost[i], 0)                # subtract edge cost (but don't go below zero)
        if (v <= 0)                                 # if account is depleted, we're done
            return(NULL)
        graph.kern[i] <<- v
