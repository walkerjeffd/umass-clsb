...
list(
  effects = list(
    delta = list(
      total = r$results$delta,
      values = r$results$deltas
    ),
    effect = list(
      total = r$results$effect,
      values = r$results$effects 
    )
  ),
  kernels = r$results$kernels,
  targets = targets,
  nodes = nodes,
  edges = edges
) %>% 
  write_json(path = "../lib/graph/test-data.json", pretty = TRUE, auto_unbox = TRUE)
```
