...
I will focus on crossings within the West Brook watershed (`HUC12 = 010802010604`).

Map shows the crossings within this watershed, color-coded by the ln(effect).
...
crossing_ids <- tbl(con, "crossings_huc") %>% 
  filter(huc12 == "010802010604") %>% 
  pull(crossing_id)

db_crossings <- tbl(con, "crossings") %>% 
  filter(id %in% crossing_ids) %>% 
  select(id, lat, lon) %>% 
  collect()
...
crossings <- read_tsv(
  ...
  filter(id %in% crossing_ids) %>% 
  rename(x = `x-coord`, y = `y-coord`) %>% 
  left_join(db_crossings, by = "id")
```

```{r basemap, cache=TRUE}
basemap <- get_googlemap(center = c(lon = mean(range(crossings$lon)), lat = mean(range(crossings$lat))), zoom = 12)
```
...
Histograms show the distribution of effects for these crossings in log and linear space.

```{r, fig.width=10, fig.height=4}
p1 <- crossings %>% 
  ggplot(aes(effect_ln)) +
  geom_histogram() +
  labs(x = "ln(Effect)", y = "# Crossings")
p2 <- crossings %>% 
  ggplot(aes(effect)) +
  geom_histogram() +
  labs(x = "Effect", y = "# Crossings")
grid.arrange(p1, p2, ncol = 2)
```

> Given this information (effect of each individual crossing), how would the user go about selecting which crossing(s) to improve?
...
Not all nodes are associated with a crossing. In fact, most are not. Non-crossing nodes exist to ensure consistent segmentation of the edges (30 m length), and to represent tributary confluences.
...
ggmap(basemap, extent = "device", darken = c(0.25, "white")) +
  ...
  geom_point(
    data = crossings,
    aes(x = lon, y = lat, color = "Crossing"),
    size = 0.5
  ) +
  scale_color_manual("", values = c("Node" = "black", "Crossing" = "red"))
```

# Scenario Results

Let's run the critical linkages tool to evaluate the single crossing, which is labeled as Target on this map.
...
Improving this single crossing, the CLSB results give a delta of `r scales::comma(round(output$results$delta))` and an effect of `r scales::comma(round(output$results$effect))`. These two metrics are the sums of individual delta and effect values for each segment in the network localized around the target crossing. The delta and effect are computed from the change in the resistance kernel due to improvement of this crossing.
...
## Relative to the Effects Distribution of All Crossings

Another way to understand the meaning of this result is in how it relates to the effects of targeting all other crossings (individually).

This figure plots the histograms of the effects for each crossing within the West Brook watershed on linear and logarithmic scales. The vertical red lines show the value of the target crossing evaluated above.
...
```{r}
ecdf_effect <- ecdf(crossings$effect)
ecdf_effect_ln <- ecdf(crossings$effect_ln)
```

A more useful way to plot this is a cumulative frequency distribution. These two plots show the empirical cumulative frequency distribution of individual effects for all crossings. The red points show the probability of the target crossing being evaluted in this section, which had an effect of `r scales::comma(round(output$results$effect))` that is located at the `r scales::percent(ecdf_effect(output$results$effect))` percentile.

```{r, fig.width=12, fig.height=4}
p1 <- data_frame(
  effect = crossings$effect,
  prob = ecdf_effect(crossings$effect)
) %>% 
...
p2 <- data_frame(
  effect_ln = crossings$effect_ln,
  prob = ecdf_effect_ln(crossings$effect_ln)
) %>% 
