...
```{r}
set.seed(1234)
targets <- sample_n(crossings, size = 1)
print(targets)
```
...
```{r, eval=FALSE}
d <- get.graph.tiles(points = targets, bandwidth = bandwidth, search = search, tilesize = tilesize, internode = internode, cellsize = cellsize, fudge = fudge, source = source, upgrades = TRUE, chatter = chatter)
```
...
First select multiple (say, four) crossings are at random.

```{r}
set.seed(1234)
targets <- sample_n(crossings, size = 4)
print(targets)
```


The results for these four crossings are:
...
for (i in seq(1, nrow(targets))) {
  ...
  delta <- c(delta, r$results$delta)
  effect <- c(effect, r$results$effect)
}
...
> If multiple target crossings are selected, the total delta and effect is the sum of the individual effects if no two crossings are located near one another and thus each crossing impacts distinct set of graph edges
